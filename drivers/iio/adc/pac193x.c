			      ...
			      long mask)
{
	...
	u64 tmp;
	s64 curr_energy;
	int fsb_index = 0; /* first signifiant bit index */
	int shift_basis = 0;
	 ...
	 */
				curr_energy = chip_info->chip_reg_data.energy_sec_acc[chan->channel];
				if (curr_energy < 0)
					curr_energy = ~curr_energy;
				fsb_index = fls64(curr_energy);
				if (fsb_index > BIT_INDEX_31)
					/* right-shifts the value in order to put first signifiant bit on the 31st position */
					*val = chip_info->chip_reg_data.energy_sec_acc[chan->channel]>>(fsb_index - BIT_INDEX_31);
				else
					*val = chip_info->chip_reg_data.energy_sec_acc[chan->channel];
				return IIO_VAL_INT;
			default:
				return -EINVAL;
			}
	...
	case IIO_CHAN_INFO_SCALE:
		 ...
		 */
			...
			if (fsb_index > BIT_INDEX_31) {
				int diff = shift_basis - (fsb_index - BIT_INDEX_31);
...
chip_info->chip_reg_data.ctrl_regs[PAC193X_CTRL_LAT_REG_OFF])>>6)]);
		...
		/* the chip's sampling rate is 2^samp_shift samples/sec */
			...
			inc =  (chip_info->chip_reg_data.vpower_acc[cnt] >> samp_shift);
			if (curr_energy >= 0) {
				if (inc >= (((s64)PAC_193X_MAX_POWER_ACC) - curr_energy))
					curr_energy = PAC_193X_MAX_POWER_ACC;
				else
					curr_energy += inc;
			} else {
				if (inc < (((s64)PAC_193X_MIN_POWER_ACC) - curr_energy))
					curr_energy = PAC_193X_MIN_POWER_ACC;
				else
					curr_energy += inc;
			}
			chip_info->chip_reg_data.energy_sec_acc[cnt] = curr_energy;
		}
	}
	/* continue with VBUS */
	for (cnt = 0; cnt < chip_info->phys_channels; cnt++) {
					...
					struct pac193x_chip_info *chip_info)
{
	...
	for_each_child_of_node(client->dev.of_node, node) {
		...
		if (of_property_read_u32(node,
			"uohms-shunt-res", &chip_info->shunts[channel_index])) {
			dev_err(&client->dev,
